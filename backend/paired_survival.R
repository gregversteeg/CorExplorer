...
coxfit <- coxph(Surv(DTE, Censor)~survtbl[,grpcol1]+survtbl[,grpcol2], data=survtbl)

fit_coeff = summary(coxfit)$coefficients[1,5]  
sink("pair_surv_tmp/coxfit.txt")
cat(fit_coeff)
sink()
...
sink("pair_surv_tmp/test.txt")
print(survtbl[,c("DTE","Censor",grpcol1,grpcol2,"relrisk"),drop=FALSE], row.names=FALSE)
sink()
...
sink("pair_surv_tmp/survival.txt")
cat(paste(sfit$surv, collapse='\n'))
sink()
sink("pair_surv_tmp/survtimes.txt")
